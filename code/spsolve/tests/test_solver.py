...
def infinite_well():
    ...
    T = 0
    N = 400
    return solver.StackedLayers(T, N, (True, -0.05), (True, -0.05), material)
...
def test_fermi_dirac():
    ...
    for E_f in np.linspace(-10, 10, 200):
        for T in np.linspace(0, 500, 500):
            assert np.all(
                solver.fermi_dirac(E_f, E, T) == 1 / (1 + np.exp((E - E_f) / k_b / T))
            )
...
def test_solve_schrodinger(infinite_well):
    ...
    for V_0 in np.linspace(-2, 2, 20):
        ...
        for n in np.arange(N) + 1:
            ...
            energies[n - 1] = (n * math.pi * h_bar) ** 2 / (
                2 * infinite_well.m_e[0] * L ** 2
            ) - V_0

        psi_test, energies_test = infinite_well.solve_schrodinger(band)

        assert np.all(
            np.abs(psi[:, 0:10]) == pytest.approx(np.abs(psi_test[:, 0:10]), 0.01)
        )
...
def test_solve_charge(infinite_well):
    ...
    for V_0 in np.linspace(-1, 1, 20):
        ...
        rho = (
            ...
            * m_e
            / (math.pi * h_bar ** 2)
            * np.dot(inner_product, -energies * (energies < 0))
        )

        assert np.all(
            rho == pytest.approx(infinite_well.solve_charge(psi, energies), 0.1)
        )


def test_solve_charge_dos(infinite_well):
    """
    Test solve_charge_dos() at T=0.
    """
    grid = infinite_well.grid
    L = infinite_well.L
    N = infinite_well.N
    ...
    for V_0 in np.linspace(-1, 1, 20):
        ...
        assert np.all(
            rho == pytest.approx(infinite_well.solve_charge_dos(-q_e * V_0 * np.ones(N)), 0.1)
        )
