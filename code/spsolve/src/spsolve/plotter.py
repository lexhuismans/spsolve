...
def plot_distributions(grid, n_e, band, psi, energies, title=None, n_e_fit=None, L=20):
    ...
    ax_pois[2].plot(grid, psi[:, 0] ** 2, label="$E_0$ = {:f} eV".format(energies[0]))
    ...
    if not isinstance(n_e_fit, type(None)):
        ...
        ax_pois[2].plot(
            grid[1:-1],
            fit[1:-1] ** 2,
            label="Analytical $E_1$ = {:f} eV".format(
                math.pi ** 2 * h_bar ** 2 / (2 * m_eff * 20 * 2) + 0.799
            ),
        )
    else:
        ...
        for n in np.arange(number_of_wavefunctions - 1) + 1:
            ax_pois[2].plot(
                grid, psi[:, n] ** 2, label="$E_{}$ = {:f} eV".format(n, energies[n])
            )

    ax_pois[2].legend(fontsize="xx-small", bbox_to_anchor=(1, 1))
    ax_pois[2].set_title("Probability ($nm^{-1}$)")
    ax_pois[2].set_xlabel("Position (nm)")
    ax_pois[2].set_ylabel("$|\psi|^2$")
...
def plot_mult_iteration(number_of_iterations=1, fit=False):
    ...
    for i in np.arange(number_of_iterations):
        ...
        phi = solver.solve_poisson(
            pois_matrix, n_e, dl, potential_left_boundary, potential_right_boundary
        )
    ...
    if fit == True:
        ...
        n_e_fit = (
            ...
            * m_eff
            / (L * math.pi * h_bar ** 2)
            * np.sin(math.pi / L * grid) ** 2
            * (E_fermi - E_n)
        )
        title = "iterations_" + str(number_of_iterations) + "_fit.png"
        plot_distributions(
            grid, n_e, phi, psi, energies, title=title, n_e_fit=n_e_fit, L=L
        )
    else:
        # PLOT #
...
def plot_optimize():
    ...
    # Poisson matrix
    pois_matrix = solver.comp_pois_matrix(dl, N, dirichlet_left, dirichlet_right)
    phi_init = solver.solve_poisson(
        pois_matrix, rho, dl, potential_left_boundary, potential_right_boundary
    )
...
def plot_poisson(grid, phi, fit=False):
    ...
    L = grid[-1] + grid[0]
    phi_analytical = (
        -K / 2 / PERMETTIVITY / 12.048 * grid ** 2
        + K * L / 2 / PERMETTIVITY / 12.048 * grid
        - 0.6
    )
